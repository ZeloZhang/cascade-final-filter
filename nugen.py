...
def nugen_weights(tray, name):
    ...
    def saveAtmWeight(frame):
        if(not frame.Has("I3MCTree")):
            print ('---NO MCTree in EvID = ', frame['I3EventHeader'].event_id)
            return
...
def nugen_truth(tray, name):
    ...
    def GetLosses(frame):
        ...
        for p in tree:
            if p.is_neutrino == True and p.location_type == dataclasses.I3Particle.LocationType.InIce and not n.isnan(p.length) and not p.length == 0:
                neutrinos.append(p)

        for p in neutrinos:
            if tree.get_daughters(p):
